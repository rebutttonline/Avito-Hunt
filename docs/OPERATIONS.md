...
Администратор также видит кнопку `🛠 Админ-панель` в Telegram. Там отображаются
пользователи, активные подписки, объявления, уведомления, отзывы и состояние
источника. При 3 и 10 последовательных ошибках источника бот отправляет тревогу,
а в 09:00 МСК — ежедневную сводку. Команда `/demo` проверяет весь пользовательский
сценарий на вымышленном объявлении без обращения к поставщику данных.
