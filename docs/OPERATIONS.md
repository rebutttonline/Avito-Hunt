...
При включённом ограниченном HTML-пилоте админ-панель дополнительно показывает
название источника, время последнего сбора и отношение принятых карточек к
полученным. Дата завершения задаётся заранее; после неё воркер переходит в статус
`expired` и больше не обращается к странице.
